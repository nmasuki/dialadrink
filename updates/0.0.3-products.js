...
function importOptions(products, done) {
    var _options = products.map(p => p.options.map(o => o.name).concat(p.quantity))
        .flatten().filter(o => o)
        .distinctBy(c => c.cleanId());
    ...
    Product.findPublished(filter)
        .exec((err, products) => {
            ...
            _products.forEach(data => {
                if (data.brand)
                    ProductBrand.model.findOne({name: data.brand})
                        .exec(function (err, brand) {
                            if (!err) {
                                var product = products.find(p => p._id.toString() == data._id)
                                product.brand = brand;
                                product.save();
                            }
                        });
